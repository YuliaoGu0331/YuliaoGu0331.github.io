---
permalink: /comments/
title: "Notes"
description: "Personal reflections and notes by Yuliao Gu."
author_profile: true
---
# Notes

Informal writing and personal reflections beyond my academic work.

<div class="list__item">
  <article class="archive__item">
    <h2 class="archive__item-title">
      <a href="{{ '/comments/phd-in-pku-4' | relative_url }}">[Ph.D. in PKU]-4</a>
    </h2>
    {% include published-date.html date="2026-09-04" %}
    <p>Opening-ceremony day and reflections on research, uncertainty, and the direct-entry Ph.D. path.</p>
  </article>
</div>

<div class="list__item">
  <article class="archive__item">
    <h2 class="archive__item-title">
      <a href="{{ '/comments/phd-in-pku-3' | relative_url }}">[Ph.D. in PKU]-3</a>
    </h2>
    {% include published-date.html date="2026-09-01" %}
    <p>Orientation meetings, research-direction conversations, and the search for a workspace at PKU.</p>
  </article>
</div>

<div class="list__item">
  <article class="archive__item">
    <h2 class="archive__item-title">
      <a href="{{ '/comments/phd-in-pku-2' | relative_url }}">[Ph.D. in PKU]-2</a>
    </h2>
    {% include published-date.html date="2026-08-31" %}
    <p>Registration day, settling into the dormitory, a walk with my parents, and my first night at PKU.</p>
  </article>
</div>

<div class="list__item">
  <article class="archive__item">
    <h2 class="archive__item-title">
      <a href="{{ '/comments/phd-in-pku-1' | relative_url }}">[Ph.D. in PKU]-1</a>
    </h2>
    {% include published-date.html date="2026-08-30" %}
    <p>The final day of summer break, the journey north, and the quiet eve of doctoral registration at Peking University.</p>
  </article>
</div>

<div class="list__item">
  <article class="archive__item">
    <h2 class="archive__item-title">
      <a href="{{ '/comments/roommates' | relative_url }}">Reflections on Shared Living at Fudan</a>
    </h2>
    {% include published-date.html date="2026-06-05" updated="2026-08-26" %}
    <p>Thoughts on communication, boundaries, and personal growth from my undergraduate years.</p>
  </article>
</div>
